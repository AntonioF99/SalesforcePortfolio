...
export default class SubscriptionExpiringWidget extends NavigationMixin(LightningElement) {
    ...
    wiredExpiring({ error, data }) {
        ...
        if (data) {
            this.expiringSubscriptions = data.map(sub => {
                const daysRemaining = this.calculateDaysRemaining(sub.Trial_End_Date__c);
                return {
                    ...sub,
