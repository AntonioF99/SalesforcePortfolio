...
export default class InvoiceOverdueDashboard extends NavigationMixin(LightningElement) {
        ...
        if (data) {
            this.overdueInvoices = data.map(invoice => {
                ...
                return {
                    ...
                    reminderBadge: hasReminders ? `${remindersSent} reminder${remindersSent !== 1 ? 's' : ''} sent` : null
                };
            });
    ...
    get totalOverdueAmount() {
        ...
        return this.overdueInvoices.reduce((sum, inv) => sum + (inv.Total_Amount__c || 0), 0);
    }
